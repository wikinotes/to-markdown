...
Input: the contents of a file in docx format
...
def markdownify(file):
    return convert_to_markdown(opendocx(file))
...
def convert_to_markdown(document):
    ...
    for element in document['doc'].getchildren()[0].getchildren():
        ...
        if element.tag == "{%s}p" % nsprefixes['w']:
            blocks+=(parse_paragraph(element,document))+"\n\n"
        
        # a table
        if element.tag == "{%s}tbl" % nsprefixes['w']:
            blocks+=parse_table(element,document)+"\n\n"
            
            
    return blocks

def parse_table(table,document):
    table_string = ""
    table_data = []
    col_count=0
                ...
                if cols.tag =="{%s}tc" % nsprefixes['w']:
                    ...
                    for el in cols.getchildren():
                        ...
                        if el.tag =="{%s}p" %nsprefixes['w']:
                            cell_data.append(parse_paragraph(el,document))
                    cell = {
...
def parse_paragraph(paragraph,document):
    #print "new paragraph"
    paragraph_style = ""
    plaintext = ""
    bold = False
                ...
                if child.tag =="{%s}drawing" % nsprefixes['w']:
                    for el in child.iter():
                        if el.tag == "{%s}pic"%nsprefixes['pic']:
                            ...
                            text=parse_picture(child,document)
...
def parse_picture(pic,document):
    picture_link=""
